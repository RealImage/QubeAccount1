...
export function ServicesCatalog() {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex rounded-md border border-slate-200 bg-slate-50 text-sm font-medium">
        <button
          onClick={() => setTab('all')}
          className={clsx('flex-1 rounded-md px-4 py-2.5', tab === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500')}
        >
          All Company Services
        </button>
        <button
          onClick={() => setTab('internal')}
          className={clsx('flex-1 rounded-md px-4 py-2.5', tab === 'internal' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500')}
        >
            ...
            {filtered.map((s) => (
              <tr key={s.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4">
                  <a href={`https://${s.accessUrl}`} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">
                    {s.accessUrl}
                  </a>
                </td>
                <td className="px-6 py-4 text-slate-600">{subscribedCount(s.id)}</td>
                <td className="px-6 py-4 text-slate-500">{s.lastUpdated.split(',')[0]}</td>
